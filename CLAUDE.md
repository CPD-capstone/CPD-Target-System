...
- **target_manager** — Intended to turn a drill from the database into commands on the HAL queue (`executeDrill`). Currently empty.

### Target actuation flow (cross-core)

Web/app code on core 0 never touches SPI directly. It posts a `TargetCommand {targetId, newState}` to the FreeRTOS `targetQueue`. `SolenoidControlTask` (pinned to core 1, priority 5) drains the queue into each `Target.desiredState`, then every 10 ms reconciles `desiredState` vs `currentState`, writing to the expander only if `MECHANICAL_BUFFER` (500 ms) has elapsed since that target's last actuation — this prevents shearing the target pins. Preserve this pattern: all target changes go through the queue.
